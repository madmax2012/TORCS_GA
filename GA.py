    ...
    def __init__(self, rep_length, popsize, sp, mut, fitfun, maxgen, cross, nr_processes, run_id , path, onlyThebest, runval):
        ...
        if self.onlyTheBest == 0:
            for i in range(self.popsize):
                self.cars.append(individual.individual(random.uniform(150,310), random.uniform(0,1), random.uniform(0,20), random.uniform(0,1), random.uniform(0,100), random.uniform(0,1), random.uniform(0,1), random.uniform(0,20), random.uniform(0,1), random.randint(5000,8000)))

        if self.onlyTheBest == 1:
            print "runing the best three agents"
            #fitness 123
            self.cars.append(individual.individual('270.74812041', '-0.01', '12.7627263658', '-0.00551337839428', '15.5977087477', '0.13674190088', '0.671465142161', '5.26213857713', '0.445765519129'))
                ...
                for i in range(0,self.tournamentSize):##select parents in tournament
                    ...
                    if self.parent2==9999999:
                        ...
                        if self.fitArray[self.randomPos] == -1:
                            ...
                            self.parent2=self.randomPos
                self.tempArray[replacer]=individual.individual(self.cars[self.parent1].values[0],self.cars[self.parent2].values[1], self.cars[self.parent1].values[2], self.cars[self.parent2].values[3], self.cars[self.parent1].values[4], self.cars[self.parent2].values[5], self.cars[self.parent1].values[6], self.cars[self.parent2].values[7],self.cars[self.parent1].values[8],self.cars[self.parent2].values[9] )
